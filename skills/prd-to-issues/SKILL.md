---
name: prd-to-issues
description: Break a PRD into independently-workable issues and write each as a local markdown file in $ISSUES_DIR. Use when the user wants to turn a PRD into a list of concrete tasks.
---
...
Ask the user for the PRD file path (e.g. `$PRD_MD`).
...
For each approved slice, write a markdown file in $ISSUES_DIR using the naming pattern `$ISSUES_DIR/NNN-short-title.md` (e.g. `$ISSUES_DIR/001-add-user-auth.md`).

Number issues starting from the next available number (check what files already exist in `$ISSUES_DIR`).
...
$PRD_MD (or whichever PRD file was used)
...
- Blocked by `$ISSUES_DIR/NNN-title.md` (if any)
